	...
	public byte[] encodingRIP(String[] ipSplit, int length, int maskingValue, int RoverID) {
		...
		//	System.out.println("if");
			...
			for (int i = 0; i < size; i++) {
				...
				System.out.println();
				System.out.println();
				System.out.println(" I/P Address " + "   NEXTHOP " + " COST ");
				System.out.println();
				for (int j = 0; j < routerValue.size(); j++) {
					if(routerValue.get(j).equals("0.0.0.0/24")) {
						
						
					}
					else {
					System.out.print(routerValue.get(j) + "       " + hopFetch.get(j) + "      " + costFetch.get(j));
					System.out.println();
					}
				}
